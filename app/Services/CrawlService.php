...
use Illuminate\Support\Facades\Http; // 1. "Браузер" (сходить по URL)
use Symfony\Component\DomCrawler\Crawler; // 2. "Скальпель" (покопаться в HTML)
use Exception; // 3. "Ловушка" для ошибок
...
class CrawlService
{
    ...
    {
        try {
            // Просто "сходить" по URL, чтобы проверить его доступность
            $response = Http::withoutVerifying()->withHeaders([
    'User-Agent' => 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36'
])->get($url);            // Можно добавить дополнительную логику, если нужно
            // Например, логировать статус-код или время ответа

        } catch (Exception $e) {
            // Если "авария", просто логируем ошибку
            report($e);
        }
    }
}
